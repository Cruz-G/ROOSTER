# %% [markdown]
# # 📊 Rule-Based Rooster Prediction (Generic)
# This notebook automates employee attendance prediction using historical booking patterns, holiday exclusion, and logic enforcement of minimum presence per week.

# %% 
# 📦 Module 1 – Clean and reshape the raw Excel roster file
import pandas as pd
from datetime import datetime

def clean_roster_excel(file_path):
    """
    ...
    df_melted = df_raw.melt(id_vars=employee_info_headers, value_vars=booking_cols,
...
# %%
# 📦 Module 2 – Generate working days for any month-year excluding holidays
def get_working_days(month, year, holiday_file):
    """
...
# %%
# 📦 Module 3 – Rule-based prediction logic
def generate_booking_predictions(df_cleaned, working_days_df, min_days_per_week=3, threshold=0.6):
    """
    ...
    def enforce_min_bookings(group):
    ...
    return schedule[['Associate ID', 'Associate Name', 'Date', 'Weekday', 'Predicted']].rename(columns={'Predicted': 'Booked'})

# %%
# 📦 Module 4 – Apply prediction output to template
from openpyxl import load_workbook
from datetime import date

def apply_predictions_to_template(predicted_df, template_path, output_path):
    """
    ...
    wb.save(output_path)
    print(f"✅ Final file with 'Y' markings saved as: {output_path}")
